 ...
 *
 * Features:
 * - Secure, KV-based dynamic session management for admin login.
 * - Integration with Cloudflare Secrets for sensitive data (passwords).
 * - Robust reverse proxy engine with redirection and cross-domain rewriting.
 * - Hardened with security headers and designed for rate limiting.
 */
...
const SESSION_EXPIRATION_SECONDS = 3600; // 管理员会话有效期: 1小时
...
async function handleRequest(request, env) {
    ...
    if (path === '/') {
        return addSecurityHeaders(new Response(getHomePage(), { headers: { 'Content-Type': 'text/html;charset=UTF-8' } }));
    }
...
async function handleAdminRoutes(request, env) {
  ...
  // 登出
  if (path === '/logout') {
    const sessionToken = getCookie(request, ADMIN_COOKIE_NAME);
    if (sessionToken) {
        // 尽力而为地删除会话，不阻塞用户
    ...
    headers.set('Set-Cookie', `${ADMIN_COOKIE_NAME}=; Path=/; Expires=Thu, 01 Jan 1970 00:00:00 GMT; HttpOnly; Secure; SameSite=Strict`);
  ...
  if (path === '/admin' && request.method === 'POST') {
      ...
      // 从环境变量(Secrets)中安全地获取密码
      if (formData.get('password') === env.ADMIN_PASSWORD) {
          // 密码正确, 创建一个无法预测的会话令牌
          const sessionToken = crypto.randomUUID();
          const expires = Date.now() + SESSION_EXPIRATION_SECONDS * 1000;
          
          await env.LINK_STORAGE.put(`session:${sessionToken}`, JSON.stringify({ expires }), {
              expirationTtl: SESSION_EXPIRATION_SECONDS
          });

          const headers = new Headers();
          headers.set('Location', `https://${url.hostname}/generate`);
          headers.set('Set-Cookie', `${ADMIN_COOKIE_NAME}=${sessionToken}; Path=/; Max-Age=${SESSION_EXPIRATION_SECONDS}; HttpOnly; Secure; SameSite=Strict`);
          return new Response(null, { status: 302, headers: headers });
      }
      return addSecurityHeaders(new Response(getAdminLoginPage('密码错误'), { status: 401, headers: { 'Content-Type': 'text/html;charset=UTF-8' } }));
  }
  ...
  // 默认显示登录页面
  if (path === '/admin') {
      return addSecurityHeaders(new Response(getAdminLoginPage(), { headers: { 'Content-Type': 'text/html;charset=UTF-8' } }));
  }
  ...
  if (path === '/generate') {
    ...
    return addSecurityHeaders(new Response(getLinkGeneratorPage(), { headers: { 'Content-Type': 'text/html;charset=UTF-8' } }));
  }
  ...
  return addSecurityHeaders(new Response('后台页面未找到', { status: 404 }));
}
...
async function handleGenerateLink(request, env) {
  ...
  if (!normalizedUrl) {
    return addSecurityHeaders(new Response(getLinkGeneratorPage('无效的目标URL'), { status: 400, headers: { 'Content-Type': 'text/html;charset=UTF-8' } }));
  }
  ...
  try {
    await env.LINK_STORAGE.put(`link:${randomPath}`, JSON.stringify(data), {
        expiration: expires ? Math.floor(expires / 1000) : undefined
    });
  } catch (err) {
    console.error('KV写入错误:', err.message);
    return addSecurityHeaders(new Response(getLinkGeneratorPage('创建链接失败，请检查KV配置'), { status: 500, headers: { 'Content-Type': 'text/html;charset=UTF-8' } }));
  }
  
  return addSecurityHeaders(new Response(getLinkGeneratedPage(link, linkPassword, expires), { headers: { 'Content-Type': 'text/html;charset=UTF-8' } }));
}
...
async function handleListLinks(request, env) {
  try {
    ...
    let links = await Promise.all(list.keys.map(async (key) => {
      ...
      if (data) {
        return { path: key.name.replace('link:', ''), target: data.target, password: data.password, created: new Date(data.created).toLocaleString('zh-CN'), expires: data.expires ? new Date(data.expires).toLocaleString('zh-CN') : '永不' };
      }
      return null;
    }));
    links = links.filter(Boolean).sort((a, b) => new Date(b.created) - new Date(a.created));
    return addSecurityHeaders(new Response(getLinkListPage(links), { headers: { 'Content-Type': 'text/html;charset=UTF-8' } }));
  } catch (err) {
    console.error('KV列表错误:', err.message);
    return addSecurityHeaders(new Response(renderPage('错误', '<h1>获取链接列表失败</h1>'), { status: 500, headers: { 'Content-Type': 'text/html;charset=UTF-8' } }));
  }
}
...
async function handleProxyRequest(request, env) {
    ...
    let data;
    try {
        data = await env.LINK_STORAGE.get(`link:${proxyId}`, { type: 'json' });
    } catch (err) {
        console.error('KV读取错误:', err.message);
        return new Response('访问链接数据时出错', { status: 500 });
    }

    if (!data) return new Response('链接已失效或不存在', { status: 404 });
    ...
    if (data.password && !isAuthenticated) {
        if (request.method === 'POST') {
            ...
            if (formData.get('password') === data.password) {
                ...
                return addSecurityHeaders(new Response(getPasswordPage(url.pathname, '密码错误'), { status: 401, headers: { 'Content-Type': 'text/html;charset=UTF-8' } }));
            }
        }
...
async function isAdminAuthenticated(request, env) {
  ...
  try {
    ...
    console.error("Session KV read failed", e);
    return false;
  }
}
...
function addSecurityHeaders(response) {
    ...
    headers.set('Content-Security-Policy', "default-src 'self'; style-src 'unsafe-inline'; form-action 'self'");
...
class UrlRewriter {
    ...
    transform(response) {
        const rewriter = new HTMLRewriter().on('a, link, img, script, form, source', this);
